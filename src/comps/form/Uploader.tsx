...
export class Uploader extends React.PureComponent<IProps, IState>{
    ...
    render = (): JSX.Element => {
        ...
        icon = props.icon || { iconKey: "file-upload" }
        ...
        return <div className="dolfo-uploader">
            ...
            {
                !props.dropArea ? <InputWrapper onClick={() => input.click()} icon={icon} style={props.wrapperStyle} label={props.label || Constants.UPLOAD_FILE_LABEL} resetFunction={this.resetFiles} value={files ? this.getFilesNameSeparated() : ""} disabled={props.disabled} required={props.required}>
                    <input type="text" readOnly value={files && value.length ? this.getFilesNameSeparated() : ""} style={props.style} />
